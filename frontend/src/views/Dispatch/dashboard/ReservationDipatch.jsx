...
import { X, MapPin, Clock, Users, CreditCard, Plus, Minus, Map, ArrowBigLeft, ArrowLeft } from 'lucide-react';
...
const ReservationFormDipatch = () => {
  ...
  // Add this state for the extras modal
  const [isExtrasModalOpen, setExtrasModalOpen] = useState(false);
...
useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    if (!firstName || !selectedDriver?.id || !selectedCategory?.name || !lastName || !email || !phone || !pickupAddress || !dropoffAddress || !selectedCategory || !price) {
      Swal.fire({
        icon: 'error',
    ...
    const formData = {
      ...
      extraCharge: extraForm.extraCharge,
      waitingMin: String(extraForm.waitingMin),
      driverPackageCharge: extraForm.driverPackageCharge,
      driverWaitingCharge: extraForm.driverWaitingCharge,
      rideCategory:selectedCategory?.name
    };

    try {
      const response = await bookingService.createDispatchBooking(formData);
      Swal.fire({
        icon: 'success',
        title: 'Success',
      ...
      navigate('/Dispatch/dashboard/reservation')
      ...
      {/* Rest of your existing JSX content remains the same */}
          ...
          <div className="bg-white rounded-lg w-full  p-8">
            <div className="flex items-start gap-1 pb-4">
             <button
              ...
              <div className="mb-8">
                <h1 className="text-3xl font-bold text-gray-900 mb-2">Create New Reservation</h1>
                <p className="text-gray-600">fill in all required information</p>
              </div>
            </div>
            <div className="grid grid-cols-1 reverse w-full xl:grid-cols-4 gap-8">
              {/* Left Column - Form */}
                    ...
                    <div>
                      ...
                      <PhoneInput
                        ...
                        inputProps={{
                          name: 'phone',
                          required: true,
                          className: `w-full px-12 text-black py-2 border rounded-lg focus:outline-none focus:ring-2 ${phoneValid
            ...
            <p className="text-gray-500 text-xs mt-1">Leave empty if not needed</p>
                ...
                <div className="space-y-4">
                  <h2 className="text-xl font-semibold text-gray-800 border-b pb-2">Drivers</h2>
                  <div className="space-y-2">
                    <select
                      name="driver"
                      className="text-black p-3 border accent-current"
                      required
                      onChange={(e) => {
                        const selected = drivers.find(driver => driver.id === parseInt(e.target.value));
                    ...
                    >
                      ...
                      {drivers.map(driver => (
                        <option key={driver.id} value={driver.id} className="p-4 text-black">
                ...
                <div className="space-y-4">
                  <h2 className="text-xl font-semibold text-gray-800 border-b pb-2">Additional Services</h2>
                  <button
                    type="button"
                    onClick={() => setExtrasModalOpen(true)}
                    className="min-w-40 bg-black text-white py-3 px-6 rounded-lg text-lg font-medium hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition duration-200"
                  >
                    Options
                  </button>
                </div>
                ...
                <div className="pt-6">
                  <button
                    ...
                    className="w-full bg-blue-600 text-white py-3 px-6 rounded-lg text-lg font-medium hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition duration-200"
                  >
                    Book Reservation
                  </button>
                </div>
        ...
        >
          <textarea
            ...
            className="text-black w-full h-32 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
          />
        ...
        {/* / Add this modal after your existing modals (before the closing </div> of your component) */}
        <Modal
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                min="0"
                step="0.01"
                value={extraForm.extraCharge}
                onChange={(e) => setExtraForm(prev => ({
                  ...prev,
            ...
            <div>
              ...
              <input
                ...
                min="0"
                step="0.01"
                value={extraForm.driverWaitingCharge}
                onChange={(e) => setExtraForm(prev => ({
                  ...prev,
                  driverWaitingCharge: parseFloat(e.target.value) || 0
                }))}
                className="text-black w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="0.00"
              />
            ...
            <div>
              ...
              <input
                ...
                min="0"
                step="0.01"
                value={extraForm.driverPackageCharge}
                onChange={(e) => setExtraForm(prev => ({
                  ...prev,
                  driverPackageCharge: parseFloat(e.target.value) || 0
                }))}
                className="text-black w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="0.00"
              />
            ...
            <div>
              ...
              <input
                type="number"
                min="0"
                value={extraForm.waitingMin}
                onChange={(e) => setExtraForm(prev => ({
                  ...prev,
                  waitingMin: parseInt(e.target.value) || 0
                }))}
                className="text-black w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="0"
              />
...
export default ReservationFormDipatch;